...
import 'package:flutter_image_compress/flutter_image_compress.dart'; // Sıkıştırma için

class PdfStudioProvider extends ChangeNotifier {
  final ImagePicker _picker = ImagePicker();
  
  List<File> _selectedImages = [];
  bool _isGenerating = false;
  String _statusMessage = ""; // Kullanıcıya ne olduğunu söylemek için
  ...
  // --- 5. GELİŞMİŞ PDF OLUŞTURMA (Sorun Çözücü) ---
  Future<void> createAndShowPdf(BuildContext context) async {
    ...
    try {
      ...
      for (int i = 0; i < _selectedImages.length; i++) {
        // Kullanıcıya bilgi ver (Örn: "Sayfa 3/10 işleniyor")
        _statusMessage = "Sayfa ${i + 1} / ${_selectedImages.length} işleniyor...";
        notifyListeners();

        File imgFile = _selectedImages[i];
        
        // --- KRİTİK NOKTA: Bellek Optimizasyonu ---
        // Resmi belleğe almadan önce sıkıştırıyoruz.
        // Genişliği 1080px'e sabitliyoruz, bu A4 için gayet yeterli.
        Uint8List? imageBytes = await FlutterImageCompress.compressWithFile(
          imgFile.absolute.path,
          minWidth: 1080, 
          minHeight: 1080,
          quality: 80, // Kaliteyi %80 yaptık, gözle görülür fark olmaz ama bellek rahatlar
        );

        // Eğer sıkıştırma başarısız olursa (nadiren), orijinali dene
        ...
        pdf.addPage(
          pw.Page(
            ...
            margin: const pw.EdgeInsets.all(20), // Kenar boşluğu bıraktık
      ...
      // --- DEĞİŞİKLİK: Printing Yerine Doğrudan Dosya Yazma ---
      // Bu yöntem çok daha kararlıdır.
